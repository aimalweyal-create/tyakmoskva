...
log = logging.getLogger("tyak_moscow")
...
def sanitize_stock(raw, sizes):
    """Приводит присланное наличие к словарю {вкус: количество (>=0)}.

    Учитываются только вкусы, которые реально есть в sizes — так админка
    не может создать "зависший" остаток для несуществующего вкуса.
    Если raw не задан (None) — наличие для товара не отслеживается вовсе
    (старое поведение: вкус всегда доступен). Пустой словарь {} тоже
    означает "наличие не отслеживается".
    """
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "ТЯК-МОСКВА: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "👑 *ТЯК-МОСКВА — Vape Shop*\n\n"
        "Добро пожаловать в ТЯК-МОСКВА! Одноразные электронные сигареты, "
        "жидкости, POD-системы и аксессуары от проверенных брендов.\n\n"
        "Нажмите кнопку ниже, чтобы открыть каталог товаров, проверить наличие и оформить заказ.\n\n"
        "18+ Продажа осуществляется только совершеннолетним покупателям.",
...
def create_order():
    ...
    items_lines = "\n".join(
        f"· {i.get('name','—')} (вкус: {i.get('size') or '—'}) × {i.get('qty', 1)}"
        for i in items
    )
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ TM-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_create_product():
    ...
    product = {
        ...
        "sizes": final_sizes,
        "stock": sanitize_stock(data.get("stock"), final_sizes),
...
def admin_update_product(pid):
    ...
    for p in items:
        if p.get("id") == pid:
            ...
            if "stock" in data:
                p["stock"] = sanitize_stock(data.get("stock"), p["sizes"])
            elif "sizes" in data:
                # вкусы изменились, а наличие не прислали — оставляем только
                # остатки по вкусам, которые всё ещё существуют у товара
                p["stock"] = {k: v for k, v in (p.get("stock") or {}).items() if k in p["sizes"]}
            if "desc" in data:
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от ТЯК-МОСКВА по заказу TM-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от ТЯК-МОСКВА по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
